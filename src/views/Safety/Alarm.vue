<template>
  <div class="fire">
    ...
    <el-row :gutter="16">
      <!-- 左侧列表 -->
      <el-col :span="19" class="leftBox between column">
        <div class="card" ref="number"></div>
        <div class="card" ref="distributed"></div>
      </el-col>
      <!-- 右侧图表 -->
      <el-col :span="5" class="chartBox between column">
        <div class="card" ref="type"></div>
        <div class="card" ref="clas"></div>
        <div class="card" ref="dealWith"></div>
      </el-col>
    </el-row>
...
export default {
  ...
  methods: {
    ...
    classChart() {
      const colorList = ['#afa3f5', '#00d488', '#3feed4', '#3bafff', '#f1bb4c', '#aff', 'rgba(250,250,250,0.5)']
      const option = {
        title: {
          subtext: '报警占比',
          x: 'center',
            ...
            label: {
              normal: {
                formatter: function (params) {
                  ...
                  switch (params.name) {
                    case '烟雾传感器':
                      str = '烟雾传感器' + params.value + '%'
                      break
                    case '火焰传感器':
                      str = '火焰传感器' + params.value + '%'
                      break
                    case '一氧化碳传感器':
                      str = '一氧化碳传感器' + params.value + '%'
                      break
                    case '震动传感器':
                      str = '震动传感器' + params.value + '%'
                      break
                    case '雨滴传感器':
                      str = '雨滴传感器 ' + params.value + '%'
                      break
                    case '蜂鸣器传感器':
                      str = '蜂鸣器传感器 ' + params.value + '%'
                      break
                  }
                  return str
                },
    ...
    // 分布
    distributedChart() {
      const dom = this.$refs.distributed
      // const uploadedDataURL = 'https://cdn.makeapie.cn/asset/get/s/data-1585799654003-46R-3f-c3.json'
      const uploadedDataURL = '../../assets/js/room.json'
      // 线条聚点
      const geoGpsMap = {
        1: [116.415, 39.915],
        2: [116.448, 39.935],
      ...
      // axios.get(uploadedDataURL, function (geoJson) {
      this.$echarts.registerMap('home', room)
      ...
      CreateChart(dom, optionXyMap01)
      // })
      // })
    },
    typeChart() {
      const loaddata = [99, 98, 100, 99, 98, 100, 99, 98]
      const option = {
        backgroundColor: '',
        title: [
          {
